...
[ -n "$TURN_INSTALL" ] && TURN_DOMAIN=$(sudo -u www-data /var/www/html/nextcloud/occ config:system:get overwrite.cli.url | sed 's#https://##;s#/##')
...
#    cd
    ...
    rm -rf "$NCPATH"/assets
    yes | sudo -u www-data php /var/www/html/nextcloud/updater/updater.phar
    download_static_script setup_secure_permissions_nextcloud -P $SCRIPTS
    bash $SECURE
